...
旧名 `resiliencx` 和示例名 `foundationx` 只允许出现在迁移文档语境中；新的默认下游集成目标是 `kernel`，生成库包括 `configx`、`observex`、`testkitx`、`postgresx`、`redisx`、`kafkax`、`natsx`、`taosx`、`ossx` 和 `clickhousex`。
...
- 不把旧 `resiliencx` / `foundationx` 叙事继续作为主身份。
...
- [基础库总标准](docs/standard/xlib-standard.md)：同步 [`resiliencx`](https://github.com/ZoneCNH/resiliencx) 的公共 API、配置、错误、健康检查、metrics、测试、安全和发布规则。
