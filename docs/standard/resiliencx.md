...
本文定义 `x.go` 基础库体系的最小生产标准。`resiliencx` 是 [`https://github.com/ZoneCNH/resiliencx`](https://github.com/ZoneCNH/resiliencx) 对应的统一仓库，同时承担 Standard Source、Go Reference Template、Generator、Harness 和 Evidence Runtime。旧 `baselib-template` 只作为迁移文档语境中的兼容名出现，不再表示独立主实现仓库。
...
- 依赖方向只能从 Business 指向 L2/L1/L0/Standard，或从具体库继承 `resiliencx`。
