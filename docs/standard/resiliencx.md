...
本文定义 `x.go` 基础库体系的最小生产标准。`resiliencx` 是 [`https://github.com/ZoneCNH/resiliencx`](https://github.com/ZoneCNH/resiliencx) 对应的统一仓库，同时承担 Standard Source、Go Reference Template、Generator、Harness 和 Evidence Runtime。当前权威标准入口是 [`docs/standard/xlib-standard.md`](xlib-standard.md)；本文件保留为历史 traceability 兼容路径。
