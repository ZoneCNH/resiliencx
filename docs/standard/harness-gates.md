...
| Adoption Check | `GOWORK=off make adoption-check` | 在渲染 downstream 仓库内验证 Repository Governance Pack、`resiliencx.lock`、本地 hooks、GitHub workflow、main ruleset、`mk/governance.mk` 和 harness gate；在标准源仓库内不要求 downstream lock，但仍验证 main ruleset 禁止 bypass 且要求 `adoption-check`、`governance-check` 和 `release-check` |
