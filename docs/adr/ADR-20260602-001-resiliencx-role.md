# ADR-20260602-001: resiliencx 合并标准、模板、Generator、Harness 与 Evidence Runtime
...
Full Goal Runtime v3.1 要求一个仓库同时提供标准文本、可编译 Go 参考模板、生成器、Harness gate 和 Evidence runtime。旧叙事把 `resiliencx` 视为外部标准源、把 `baselib-template` 视为实现仓库，导致 README、docs、.agent 和 release Evidence 出现身份漂移。

## 决策

`resiliencx` 是唯一主身份，承担五类职责：
...
- README 必须明确五类职责。
- `docs/standard/module-boundary.md` 不再禁止 `resiliencx` 拥有 generator、Harness 或 Evidence 实现。
